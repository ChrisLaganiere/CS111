...
schedule(void)
{
  ...
  10008f:	a1 7c 9f 10 00       	mov    0x109f7c,%eax
	while (1) {
		pid = (pid + 1) % NPROCS;
...
schedule(void)
{
  ...
  1000a4:	83 b8 1c 92 10 00 01 	cmpl   $0x1,0x10921c(%eax)
  1000ab:	75 ee                	jne    10009b <schedule+0xf>
			run(&proc_array[pid]);
  1000ad:	83 ec 0c             	sub    $0xc,%esp
  1000b0:	05 d4 91 10 00       	add    $0x1091d4,%eax
  1000b5:	50                   	push   %eax
...
interrupt(registers_t *reg)
{
  ...
  1000c4:	83 ec 1c             	sub    $0x1c,%esp
	...
	// registers into the 'current' process descriptor.
	current->p_registers = *reg;
  1000c7:	8b 1d 7c 9f 10 00    	mov    0x109f7c,%ebx
...
interrupt(registers_t *reg)
{
  ...
  1000fc:	b8 70 92 10 00       	mov    $0x109270,%eax
  ...
  10012a:	81 c2 d4 91 10 00    	add    $0x1091d4,%edx
  ...
  100145:	83 c0 0a             	add    $0xa,%eax
	...
	dest_stack_top = PROC1_STACK_ADDR + (dest->p_pid * PROC_STACK_SIZE);
	dest_stack_bottom = dest_stack_top - (src_stack_top - src_stack_bottom);
  100148:	8b b7 d4 91 10 00    	mov    0x1091d4(%edi),%esi
	// and then how to actually copy the stack.  (Hint: use memcpy.)
	// We have done one for you.
  ...
  100172:	89 af d4 91 10 00    	mov    %ebp,0x1091d4(%edi)
	src_stack_bottom = src->p_registers.reg_esp;
	dest_stack_top = PROC1_STACK_ADDR + (dest->p_pid * PROC_STACK_SIZE);
	dest_stack_bottom = dest_stack_top - (src_stack_top - src_stack_bottom);
	// YOUR CODE HERE: memcpy the stack and set dest->p_registers.reg_esp
  ...
  100194:	ff 35 7c 9f 10 00    	pushl  0x109f7c
  ...
  1001a4:	a1 7c 9f 10 00       	mov    0x109f7c,%eax
  ...
  1001c0:	c7 40 50 ff ff ff ff 	movl   $0xffffffff,0x50(%eax)
		...
		if (current->blocked_proc > 0) {
		  proc_array[current->blocked_proc].p_state = P_RUNNABLE;
  1001c7:	c7 82 1c 92 10 00 01 	movl   $0x1,0x10921c(%edx)
  1001ce:	00 00 00 
		  proc_array[current->blocked_proc].p_registers.reg_eax = current->p_exit_status;
  1001d1:	89 8a f4 91 10 00    	mov    %ecx,0x1091f4(%edx)
		  current->blocked_proc = -1;
  ...
  1001e3:	a1 7c 9f 10 00       	mov    0x109f7c,%eax
  ...
  1001fc:	8d 9a dc 91 10 00    	lea    0x1091dc(%edx),%ebx
  ...
  100217:	8b 92 20 92 10 00    	mov    0x109220(%edx),%edx
		        proc_array[p].p_state = P_EMPTY;
  ...
  100229:	89 8a 24 92 10 00    	mov    %ecx,0x109224(%edx)
...
start(void)
{
  ...
  100248:	68 d4 91 10 00       	push   $0x1091d4
  10024d:	e8 4a 03 00 00       	call   10059c <memset>
  100252:	b8 d4 91 10 00       	mov    $0x1091d4,%eax
  ...
  100275:	c7 05 7c 9f 10 00 28 	movl   $0x109228,0x109f7c
  10027c:	92 10 00 
  ...
  100287:	ff 35 7c 9f 10 00    	pushl  0x109f7c
  ...
  1002c9:	50                   	push   %eax
  1002ca:	50                   	push   %eax
  1002cb:	a1 7c 9f 10 00       	mov    0x109f7c,%eax
  ...
  1002da:	a1 7c 9f 10 00       	mov    0x109f7c,%eax
...
segments_init(void)
{
  ...
  1002f8:	b8 14 97 10 00       	mov    $0x109714,%eax
...
segments_init(void)
{
  ...
  100319:	88 15 cc 1a 10 00    	mov    %dl,0x101acc
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  10031f:	ba 7c 97 10 00       	mov    $0x10977c,%edx
segments_init(void)
{
	int i;
...
segments_init(void)
{
  ...
  100342:	c7 05 18 97 10 00 00 	movl   $0x80000,0x109718
  100349:	00 08 00 
	kernel_task_descriptor.ts_ss0 = SEGSEL_KERN_DATA;
  10034c:	66 c7 05 1c 97 10 00 	movw   $0x10,0x10971c
  100353:	10 00 
	...
	for (i = 0; i < sizeof(interrupt_descriptors) / sizeof(gatedescriptor_t); i++)
		SETGATE(interrupt_descriptors[i], 0,
  100355:	66 89 0c c5 7c 97 10 	mov    %cx,0x10977c(,%eax,8)
  10035c:	00 
...
run(process_t *proc)
{
  100460:	8b 44 24 04          	mov    0x4(%esp),%eax
	current = proc;
  100464:	a3 7c 9f 10 00       	mov    %eax,0x109f7c
